...
See [phase17-k3s-migration-memo.md](phase17-k3s-migration-memo.md). Move HomeNews off the single-host docker-compose stack onto the home k3s cluster managed by `arch-infra` (Argo CD GitOps), the same path llmgw took on 2026-05-10. Tasks 104-118: Helm chart at `deploy/chart/` (StatefulSet db + Deployment api/web + two Ingresses), matrix GHA build to GHCR with `yq` bumping both image tags in arch-infra, `pg_dump | pg_restore` data migration of the prod database, cutover behind `homenews.arch.internal` + `homenews-api.arch.internal`, and Loki-based verification that closes Phase 16 Task 103. Cluster reference: [k3s-migration/02-K3S_REFERENCE.md](k3s-migration/02-K3S_REFERENCE.md).

### Phase 18 — UI shell redesign (PLANNED)
See [ui-shell-memo.md](ui-shell-memo.md) for the locked design. Supersedes Phase 7's centered top-nav newsroom-workstation layout with a single-sidebar app shell inspired by Claude.ai (clean, hairline-only, no card chrome) and VSCode's collapsible accordion sidebar pattern. **No top header anywhere.** Sidebar has two shapes: **Shape A** (top-level pages — primary nav + page-specific contextual zone + Settings footer) and **Shape B** (sub-pages — Back at top + sub-controls). New top-level **Pipeline** page lifts run trace + history out of the dashboard. Tasks 119-129: install `shadcn/ui` Sidebar + Collapsible primitives, build the `AppSidebar` Shape A/B route-aware swap, migrate `layout.tsx`, build the new `/pipeline` page, move dashboard filters into the sidebar contextual zone, flatten article rows (no card chrome), migrate Settings sub-sidebar from parallel column to Shape B replacement, apply empty Back-only Shape B to `/article/[id]`, render `/search` + `/highlights` in Shape A with empty contextual zones, visual pass, smoke + changelog. Visual identity (warm-dark/amber, Fraunces + Geist + Geist Mono) carries over from Phase 7 unchanged.
